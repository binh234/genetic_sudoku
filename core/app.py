...
class App:
    ...
    def render(self, text, option=RenderOption.NORMAL):
        ...
        if option != RenderOption.ONLY_TEXT:
            ...
            self.ui.drawRemainBoard(given.bestCandidate.gene)
            given.updateDuplicateValues()
            if given.bestCandidate.fitness == GOAL:
        ...
        if option == RenderOption.FOUNDED:
            self.save("./solutions/" + self.puzzle, given.bestCandidate.gene)
    ...
    def write(self):
        if not self.writing:
            ...
            try:
                ...
                savetxt(saveFile, given.bestCandidate.gene.reshape(DIGIT_NUMBER, DIGIT_NUMBER), fmt='%d')
                saveFile.close()
            except:
                return
            self.clear()
    ...
    def press(self, event):
        ...
        if event.char in list(map(chr,range(ord("1"),ord("9")+1))):
            given.bestCandidate.gene[row,col] = int(event.char)
            self.ui.drawRemainBoard(given.bestCandidate.gene)
            given.updateDuplicateValues()
            self.ui.drawDuplicateBg()
        else:
